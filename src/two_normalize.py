...
# reshape so each SC bam name is matched row to hg bam - so can iterate through and hvae everything in order
#	that matches the order of the mapped bam files below
#mapped_stats_files = np.reshape( mapped_stats_files, (-1,2) )
if len(mapped_stats_files)>1:
    mapped_stats_files = np.reshape( mapped_stats_files, (-1,2) )
print("mapped_stats_files: \n")
print(mapped_stats_files)
...
if len(mapped_stats_files)>1:
    for sample_counter in range(len(mapped_stats_files)):
        ...
        mm10_file_reads = pd.read_csv(mapped_stats_files[sample_counter], sep='\t', header=None, skiprows=12)
        # split the 7th row by whitespace to pull out just the number of reads and save
        mapped_reads_per_file[sample_counter][0] = mm10_file_reads[0][0].split()[1]
        # read in the hg file and skip all the header rows
